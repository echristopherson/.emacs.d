;;; ir-gray_EAC-theme.el -- modification of:
...
;; load-theme' and select "ir-gray_EAC".
...
(deftheme ir-gray_EAC "EAC's customized port of ir-black theme")

(let ((*background-color*   "#1C1C1C")
      (*brown*              "#E64")
      (*comments*           "#7C7C7C")
      (*constant*           "#99CC99")
      (*current-line*       "#151515")
      ;(*cursor-block*       "#FFFFFF") ; TODO: This isn't referenced here.
      ;(*cursor-underscore*  "#FFFAAA") ; TODO: This is the value used by block cursor, so it should be renamed.
      (*cursor-underscore*  "#FF0000") ; TODO: I might get sick of red eventually. I'm starting to like the shade of gray used in Gmail (the original color-theme-irblack.el uses #888888, but that doesn't quite suit me).
      (*keywords*           "#96CBFE")
      (*light-purple*       "#FFCCFF")
      (*line-number*        "#3D3D3D")
      (*method-declaration* "#FFD2A7")
      (*mode-line-bg*       "#202020") ; TODO: this color isn't what I want. I've commented out all uses of it except for org-clock-overlay. I guess this means Emacs will use the default mode-line-bg, which happens to be what the 23.x color-theme-irblack uses.
      (*mode-line-fg*       "#CCCCCC") ; TODO: see above line
      (*normal*             "#F6F3E8")
      (*number*             "#FF73FD")
      (*operators*          "#FFFFB6") ; TODO: see below
      (*red*                "#FF6C60")
      (*red-light*          "#FFB6B0")
      (*regexp*             "#E9C")
  ...
  (custom-theme-set-faces
   ...
   ;`(header-line ((t (:background, *mode-line-bg* :foreground, *normal*)))) ;; info header ; REMOVE?
   ...
   ;; font-lock
   `(font-lock-builtin-face ((t (:foreground, *operators*)))) ; TODO: This includes :keywords, which are highlighted differently in the theme I'm trying to emulate.
   ...
   ;`(mode-line ((t (:background, *mode-line-bg* :foreground, *mode-line-fg*)))) ; REMOVE?
   ;; TODO
   ;`(mode-line-inactive ((t (:background, *mode-line-bg* :foreground, *background-color*)))) ; REMOVE?
   ;; TODO
   ...
   `(org-clock-overlay ((t (:foreground, *mode-line-bg* :background, *string*)))) ; TODO: Keep for now, but determine whether to continue keeping. I haven't really used org mode yet and don't know what the clock looks like.
...
(provide-theme 'ir-gray_EAC)
;;; ir-gray_EAC-theme.el ends here
